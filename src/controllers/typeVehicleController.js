...
const getId = async (req, res) => {
  try {
    ...
    const response = await typeVehicleService.getId(_id);
...
const create = async (req, res) => {
  try {
    const body = req.body;
    const response = await typeVehicleService.create(body);
    res.send({
      statusCode: 201,
...
const update = async (req, res) => {
  try {
    const { _id } = req.params;
    const response = await typeVehicleService.update(_id, req.body);
    if (response === null) {
...
const deleteId = async (req, res) => {
  try {
    const { _id } = req.params;
    const response = await typeVehicleService.deleteId(_id);
    if (response === null) {
      return res.send({
        statusCode: 404,
